...
- **OBS 連携**: 表示ボードは背景色をカスタマイズ可能で、OBS などの配信ソフトウェアで簡単にクロマキー合成できます（グリーン/マゼンタ/ブルースクリーン対応）。
...
- **背景色カスタマイズ (NEW!)**: Electron版ではGUIで、Web版ではYAML/コマンドラインで背景色を自由に変更可能（デフォルト: マゼンタ `#ff55ff`）。
...
初めてElectronアプリを起動すると、以下の流れで初期設定をガイドします:

1. **設定ウィンドウが自動的に開きます**
2. **ウェルカムダイアログが表示されます**（推奨ワークフローの説明）
3. 操作パネルも同時に開きます（設定完了後に使用）
...
2回目以降の起動では、設定ウィンドウは開かず、操作パネルのみが起動します。
...
6. **マスター/スレーブ制御 (Phase 3)**: **Electronクライアントもブラウザクライアントと同じマスター/スレーブロジックに従います**
   - Electron版クライアントに特別な優先権はなく、最初に接続したクライアント（Electronまたはブラウザのどちらでも）がマスターになる
   - トークンベースの認証と5秒間の再接続猶予期間が適用される
   - 複数のElectronインスタンスを起動した場合や、Electron/ブラウザの混在環境でも柔軟に対応可能
7. **初回起動ガイド (Phase 5)**: 初回起動時に設定ウィンドウとウェルカムダイアログを自動表示
   - マーカーファイル (`~/.config/baseball_broadcast_board/.first_run_complete`) で初回起動を検出
   - 設定ウィンドウを自動的に前面表示 (`show: false` + `ready-to-show` イベント)
   - ウェルカムダイアログで推奨ワークフローを案内
   - 2回目以降は操作パネルのみ起動
...
5. キーカラータイプ: 背景色に応じて選択（デフォルト: マゼンタ、推奨: グリーン）
6. スコアボードが背景色なしで表示されます

**💡 背景色のカスタマイズ**:
- **Electron版**: 設定ウィンドウ (`Ctrl/Cmd+,`) の「🎨 表示ボード設定」カードで変更
- **Web版**: YAML/コマンドラインで設定（詳細は[初期設定ファイルの生成](#初期設定ファイルの生成)を参照）
...
**💡 背景色の設定**: `board_background_color`フィールドで表示ボードの背景色を指定できます（OBSクロマキー用）。
- **省略可**: 省略した場合はデフォルト値 `#ff55ff` (マゼンタ) が使用されます
- **推奨色**: グリーン `#00ff00` / マゼンタ `#ff00ff` / ブルー `#0000ff`
...
# 背景色も指定する場合
npm run init -- -t "夏季大会" -i 7 --teams "A,B,C,D,E" --bg-color "#00ff00"
```
...
- `last_inning`: 最終イニング
- `board_background_color`: 表示ボードの背景色（OBSクロマキー用、hex形式、例: `#00ff00`）
